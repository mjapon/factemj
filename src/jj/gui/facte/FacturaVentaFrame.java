...
import jj.gui.merc.unid.IListenerSelectUnity;
...
public class FacturaVentaFrame extends BaseFrame implements IListenerSelectUnity{
    ...
    public boolean checkAperturaCaja(){
        ...
        try{
            Date today = new Date();
            if (!cajasController.existeCajaAbierta(today)){
                if (cajasController.existeCajaAbiertaMenorFecha(today)){
                    Cajas caja = cajasController.getCajaAbiertaMenorFecha(today);
                    showMsg(" Existe una caja anterior no cerrada (fecha:"+FechasUtil.format(caja.getCjFecaper())+" ), debe cerrar esta caja antes de empezar el dia ");
                    resultAperturaCaja = false;
                }
                else{
                    showMsg("Primero debe hacer apertura de caja para empezar ha vender");
                }
            }
            else{
    ...
    private void initComponents() {
        ...
        jBtnPrecios.setFont(new java.awt.Font("Arial", 1, 14)); // NOI18N
        ...
        jBtnDescuentos.setFont(new java.awt.Font("Arial", 1, 14)); // NOI18N
        jBtnDescuentos.setIcon(new javax.swing.ImageIcon(getClass().getResource("/jj/gui/icons/icons8-price__comparison.png"))); // NOI18N
    ...
    private void jBtnPreciosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnPreciosActionPerformed
        ...
        try{
            ...
            FilaArticulo filaArticulo = new FilaArticulo(artId, codBarra, nombre, precioCompra, precioVenta, precioMin, iva, inventario, categoria, tipo, catid);
